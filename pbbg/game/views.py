...
class ActionsView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            is_owner = Player.objects.filter(owner=request.user, identity_id=player_identity[0].id)
            action = request.data['action']
        except (AttributeError, KeyError) as e:
            return Response(data="You must specifiy both an action and a player name.", status=status.HTTP_400_BAD_REQUEST)
        except UnboundLocalError as e:
            return Response(data="You must specify an action and a player name that exist.", status=status.HTTP_400_BAD_REQUEST)
        ...
        if is_owner.count() == 0:
            return Response(data="You do not have permission to use this character.", status=status.HTTP_400_BAD_REQUEST)
